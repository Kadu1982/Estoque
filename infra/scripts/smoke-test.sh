...
echo "[1/9] Login"
AUTH_JSON=$(curl -sS -X POST "$BASE_URL/api/v1/auth/login" \
...
SUPPLIER_JSON=$(curl -sS -X POST "$BASE_URL/api/v1/suppliers" \
...
ITEM_JSON=$(curl -sS -X POST "$BASE_URL/api/v1/items" \
  -H "$AUTH_HEADER" -H "Content-Type: application/json" \
  -d "{
    \"code\":\"ITM-$RAND\",
...
REQ_JSON=$(curl -sS -X POST "$BASE_URL/api/v1/requisitions" \
  -H "$AUTH_HEADER" -H "Content-Type: application/json" \
  -d "{
    \"urgency\":\"NORMAL\",
...
APPROVE_JSON=$(curl -sS -X PATCH "$BASE_URL/api/v1/requisitions/$REQ_ID/approve" -H "$AUTH_HEADER")
...
ORDER_JSON=$(curl -sS -X POST "$BASE_URL/api/v1/orders" \
  -H "$AUTH_HEADER" -H "Content-Type: application/json" \
  -d "{
    \"code\":\"PO-$RAND\",
...
WAREHOUSE_ID=$(curl -sS "$BASE_URL/api/v1/lookups/warehouses" -H "$AUTH_HEADER" | jq -r '.[0].id')
if [[ -z "$WAREHOUSE_ID" || "$WAREHOUSE_ID" == "null" ]]; then
  echo "Warehouse lookup failed"; exit 1
fi

echo "[8/9] Receive order"
RECEIVE_JSON=$(curl -sS -X POST "$BASE_URL/api/v1/orders/$ORDER_ID/receive" \
  -H "$AUTH_HEADER" -H "Content-Type: application/json" \
  -d "{
...
STOCK_JSON=$(curl -sS "$BASE_URL/api/v1/stock" -H "$AUTH_HEADER")
...
DASH_JSON=$(curl -sS "$BASE_URL/api/v1/dashboard/summary" -H "$AUTH_HEADER")
